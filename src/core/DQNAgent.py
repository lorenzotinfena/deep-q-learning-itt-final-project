...
class DQNAgent:
	...
	def __init__(self, env, path=None):
		...
		"""
		self._env = env
		self._nn = CustomNeuralNetwork([env.observation_space.shape[0], 5, 5, env.action_space.n], path)
	...
	def start_episode(self, discount_factor, learning_rate, exploration_epsilon: float = 0):
		...
		while not done:
			...
			action = self._env.action_space.sample()  if np.random.uniform(0, 1) < exploration_epsilon else np.argmax(q_values_predicted)
	...
	def start_episode_and_evaluate(self, discount_factor: float, learning_rate: float, exploration_epsilon: float = 0, render=False, optimize=True):
		...
		while not done:
			...
			total_reward += reward
			steps += 1
			costs.append(self._nn.cost_function(q_values_predicted, q_values_target))
