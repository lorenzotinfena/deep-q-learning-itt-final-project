...
class NeuralNetwork:
    def __init__(self, n_neurons: np.array, path: str = None):
        ...
        if path == None:
            self._weights = [np.random.uniform(low=-0.5, high=0.5, size=(n_neurons[i+1], n_neurons[i]+1)) for i in range(len(n_neurons) - 1)]
        else:
            with open(path, 'rb') as file:
                self._weights = pk.load(file)
    ...
    def predict(self, input: np.ndarray) -> np.ndarray:
		...
		"""
        for weights, activation_function in zip(self._weights, self._activation_functions):
    ...
    def backpropagate(self, z: list[np.ndarray], a: list[np.ndarray], target_output: np.array, learning_rate: float):
        ...
        for weights, z_layer, a_layer, activation_function_derivative in reversed(list(zip(self._weights[1:], z[1:-1], a[1:-1], self._activation_functions_derivative[:-1]))):
        ...
        self._weights[0] -= learning_rate * gradients_z.reshape(-1, 1).dot(np.append(a[0], 1).reshape(1, -1))
        
        # check weights integrity
        if any([any([any(np.isnan(weights)) for weights in weights]) for weights in self._weights]):
            raise Exception('NaN weights')
        
    def save(self, path: str):
        with open(path, "wb") as file:
            pk.dump(self._weights, file)